...
namespace Kmmp.Core.MqFramework.RocketMQ.Consumers
{
    ...
    public class RocketMQOrderReceiver : OrderConsumerClient, IMessageReceiver, IBroadcastReceiver
    {
        /// <summary>
        /// 构造函数
        /// </summary>
        ...
        private class OrderReceiverMessageListener : MessageOrderListener
        {
            ...
            public override OrderAction consume(Message message, ConsumeOrderContext context)
            {
                ...
                count++;
                //Console.WriteLine($"于{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff")}接收到顺序消息,count:{count}, 当前线程ID:{ Thread.CurrentThread.ManagedThreadId},Topic:{message.getTopic()},Tag:{message.getTag()},key:{message.getKey()},MsgID:{message.getMsgID()},typeFullName:{base64BodyTypeFullName}");
                Console.WriteLine($"于{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff")}接收到顺序消息,count:{count}, 当前线程ID:{ Thread.CurrentThread.ManagedThreadId},Topic:{message.getTopic()},Tag:{message.getTag()},key:{message.getKey()},MsgID:{message.getMsgID()}");
                try
                {
