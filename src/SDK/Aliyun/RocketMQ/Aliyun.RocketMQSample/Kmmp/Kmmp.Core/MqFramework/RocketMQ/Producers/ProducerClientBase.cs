...
namespace Kmmp.Core.MqFramework.RocketMQ.Producers
{
    ...
    public abstract class ProducerClientBase : RocketMQClientBase
    {
        ...
        protected ProducerClientBase(string accessKeyId, string accessKeySecret, string nameSrvAddr, string topic, string groupId, string logPath)
        ...
        protected Message ComposeMessage(object body, string tag = "", string key = "")
        {
            ...
            //base64BodyTypeFullName = Convert.ToBase64String(Encoding.UTF8.GetBytes("SayHello"));
            message.putUserProperties("BodyTypeFullName", base64BodyTypeFullName);

            message.setKey(key);

            //Console.WriteLine($"BodyTypeFullName:{base64BodyTypeFullName},BodyTypeFullName:{message.getUserProperties("BodyTypeFullName")}");
